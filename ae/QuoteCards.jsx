    ...
    function build(thisObj) {
        ...
        function doScan() {
            ...
            if (nTarget && withSpeaker === 0) {
                note += "  |  EVERY CARD WILL COME OUT WITH NO NAME: " +
                        (trim(quotes[0].speakerColumn) !== ""
                            ...
                            : "your quote file has no speaker column - add one headed " +
                              "\"المتحدث\" or \"Speaker\"") +
                        ". The name line is cleared rather than left as the template " +
                        "had it, because that name belongs to a real person who did not " +
                        "say this.";
                ...
                note += "  |  your file has " + withSpeaker + " speaker name(s) but \"Name layer\" " +
                        ...
                        "the template's name, which belongs to somebody else.";
            }
        ...
        function doCreate() {
            ...
            try {
                ...
                for (var i = 0; i < plan.length; i++) {
                    ...
                    cleared += writeSideText(nTarget, "name", row.quote.speaker, mapping, log);
                    cleared += writeSideText(rTarget, "title", row.quote.role, mapping, log);
            ...
            alert("Quote Cards\n\n" +
                  ...
                  "Name    -> " + (nTarget
                        ? nTarget.comp.name + " / " + nTarget.layer.name +
                          (cleared > 0 ? "   (" + cleared + " line(s) CLEARED - nothing in the " +
                                         "quote list says who they are)" : "")
                        : "not touched") +
                  "\n" +
         ...
         *
         * With nothing to write, the layer is CLEARED rather than left alone.
         * The template's own name is a real person's name: keeping it puts
         * that person's name and job under a different person's face, and the
         * card looks finished, so nobody checks it. An empty line is obviously
         * unfinished, which is the whole difference.
         *
         * Not wanting that is what "(leave the name alone)" in the layer list
         * is for - picking a layer here says this line is yours to manage.
         */
        function writeSideText(target, what, value, mapping, log) {
            if (!target) { return 0; }
            var layer = mapping[target.comp.id].layer(target.index);
            if (trim(value || "") === "") {
                if (setLayerText(layer, "", log)) {
                    log.push("    " + what + " CLEARED on " + layer.name + " - the quote list " +
                             "does not say who this is, and the template's " + what +
